    ...
    result=OrderSend(Symbol(),OP_BUY,PP_Lot_Size,Ask,PP_Slippage,0,0,"Project Pips",PP_MagicNumber,0,Blue);
        if(result>0)
        {
         ...
         if(OrderSelect(result,SELECT_BY_TICKET))
            opening = (OrderModify(OrderTicket(),OrderOpenPrice(),NormalizeDouble(TheStopLoss,Digits),NormalizeDouble(TheTakeProfit,Digits),0,Green));
        }
}
...
bool opening;      
   ...
   result=OrderSend(Symbol(),OP_BUYLIMIT,PP_Lot_Size,Ask-PP_PullBack_Long*MyPoint,PP_Slippage,0,0,"Project Pips",PP_MagicNumber,et,Blue);
    if (result > 0)
    {
         ...
         if(OrderSelect(result,SELECT_BY_TICKET))
            opening = OrderModify(OrderTicket(),OrderOpenPrice(),NormalizeDouble(TheStopLoss,Digits),NormalizeDouble(TheTakeProfit,Digits),et,Green);
        }
 }
...
void closeBuyTrade()
{
   bool closing;
      RefreshRates();
      closing = OrderClose(OrderTicket(),OrderLots(),Bid,PP_Slippage,Green);  
}
...
void closeSellTrade()
{
   bool closing;
      RefreshRates();
      closing = OrderClose(OrderTicket(),OrderLots(),Ask,PP_Slippage,Red);    
}
...
void DeletePendingOrder()
{
   bool deleting;
      RefreshRates();
      deleting = OrderDelete(OrderTicket()); 
}
               ...
               if(Bid-OrderOpenPrice()>MyPoint*PP_Trailing_Start)
                 {
                  if(OrderStopLoss() + MyPoint*PP_Trailing_Step < Bid - MyPoint*PP_Trailing_Stop)
                    {
                     bool modifying;
                     modifying = OrderModify(OrderTicket(),OrderOpenPrice(),NormalizeDouble((Bid-PP_Trailing_Stop*MyPoint),Digits),OrderTakeProfit(),0,Green);
               ...
               if((OrderOpenPrice()-Ask)> MyPoint*PP_Trailing_Start)
                 {
                  if((OrderStopLoss() - MyPoint*PP_Trailing_Step > (Ask + MyPoint*PP_Trailing_Stop)) || (NormalizeDouble(OrderStopLoss(), Digits)==0))
                    {
                     bool modifying;
                     modifying = OrderModify(OrderTicket(),OrderOpenPrice(),NormalizeDouble((Ask+MyPoint*PP_Trailing_Stop),Digits),OrderTakeProfit(),0,Red);
               ...
               if(Bid-OrderOpenPrice()>= PP_ATR_Trail_Start*MyPoint)
                 {
                  if(OrderStopLoss()<Bid-ATR_trailing_stop)
                    {
                     bool modifying;
                     modifying = OrderModify(OrderTicket(),OrderOpenPrice(),NormalizeDouble((Bid-ATR_trailing_stop),Digits),OrderTakeProfit(),0,Green);
               ...
               if(OrderOpenPrice()-Ask >= MyPoint*PP_ATR_Trail_Start)
                 {
                  if((OrderStopLoss()> Ask+ATR_trailing_stop) || (OrderStopLoss()==0))
                    {
                     bool modifying;
                     modifying = OrderModify(OrderTicket(),OrderOpenPrice(),NormalizeDouble((Ask+ATR_trailing_stop),Digits),OrderTakeProfit(),0,Red);
                  ...
                  if(OrderStopLoss()< PeriodLowest - PP_HiLo_Trail_Offset*MyPoint)
                   {
                    ...
                    {
                     bool modifying;
                     modifying = OrderModify(OrderTicket(),OrderOpenPrice(),NormalizeDouble((PeriodLowest - PP_HiLo_Trail_Offset*MyPoint),Digits),OrderTakeProfit(),0,Green);
                  ...
                  if(OrderStopLoss()> PeriodHighest + PP_HiLo_Trail_Offset*MyPoint)
                   {
                    ...
                    {
                     bool modifying;
                     modifying = OrderModify(OrderTicket(),OrderOpenPrice(),NormalizeDouble((PeriodHighest + PP_HiLo_Trail_Offset*MyPoint),Digits),OrderTakeProfit(),0,Red);
                  ...
                  if(OrderStopLoss()< iLow (NULL,PP_Candle_trail_Time,PP_Candle_trail_Shift) - PP_Candle_trail_Offset*MyPoint)
                   {
                    ...
                    {
                     bool modifying;
                     modifying = OrderModify(OrderTicket(),OrderOpenPrice(),NormalizeDouble((iLow (NULL,PP_Candle_trail_Time,PP_Candle_trail_Shift) - PP_Candle_trail_Offset*MyPoint),Digits),OrderTakeProfit(),0,Green);
                  ...
                  if(OrderStopLoss()> iHigh (NULL,PP_Candle_trail_Time,PP_Candle_trail_Shift) + PP_Candle_trail_Offset*MyPoint)
                   {
                    ...
                    {
                     bool modifying;
                     modifying = OrderModify(OrderTicket(),OrderOpenPrice(),NormalizeDouble((iHigh (NULL,PP_Candle_trail_Time,PP_Candle_trail_Shift) + PP_Candle_trail_Offset*MyPoint),Digits),OrderTakeProfit(),0,Red);
         ...
         if(OrderType()==OP_BUY)  
          {
           ...
           if(iSAR(NULL,PP_PSAR_Trail_Time,PP_PSAR_Trail_Step,PP_PSAR_Trail_Maxstep,PP_PSAR_Trail_Candles_Back) < Ask)
           {
            ...
            {
             bool modifying;
             modifying = OrderModify(OrderTicket(),OrderOpenPrice(),NormalizeDouble((iSAR(NULL,0,PP_PSAR_Trail_Step,PP_PSAR_Trail_Maxstep,PP_PSAR_Trail_Candles_Back)),Digits),OrderTakeProfit(),0,Green); 
            }
          }
          }
           }
         ...
         {    
          if(OrderOpenPrice()-Ask >= MyPoint*PP_PSAR_Trail_Start)
          {
           if(iSAR(NULL,PP_PSAR_Trail_Time,PP_PSAR_Trail_Step,PP_PSAR_Trail_Maxstep,PP_PSAR_Trail_Candles_Back) > Ask)
           {
             ...
             {
              bool modifying;
              modifying = OrderModify(OrderTicket(),OrderOpenPrice(),NormalizeDouble((iSAR(NULL,0,PP_PSAR_Trail_Step,PP_PSAR_Trail_Maxstep,PP_PSAR_Trail_Candles_Back)),Digits),OrderTakeProfit(),0,Red);
                 }
             }
             }
          }
         ...
         if(iATR(NULL,PP_ATR_PS_Time,14,2) < PP_ATR_PS_level && iATR (NULL,PP_ATR_PS_Time,14,1) > PP_ATR_PS_level)
          {
           if(OrderStopLoss() < Bid-PP_ATR_PS_stop*MyPoint)
            {
             bool modifying;
             modifying = OrderModify(OrderTicket(),OrderOpenPrice(),NormalizeDouble((Bid-PP_ATR_PS_stop*MyPoint),Digits),OrderTakeProfit(),0,Green);   
            }
          }
           }
         else
          ...
          if(iATR(NULL,PP_ATR_PS_Time,14,2) < PP_ATR_PS_level && iATR (NULL,PP_ATR_PS_Time,14,1) > PP_ATR_PS_level)
            {
               ...
               {
                bool modifying;
                modifying = OrderModify(OrderTicket(),OrderOpenPrice(),NormalizeDouble((Ask+PP_ATR_PS_stop*MyPoint),Digits),OrderTakeProfit(),0,Red);
                }
             }
          }
        }  
         ...
         if(iRSI(NULL,PP_RSI_PS_Time,14,PRICE_CLOSE,2) > PP_RSI_PS_high && iRSI(NULL,PP_RSI_PS_Time,14,PRICE_CLOSE,1) < PP_RSI_PS_high) 
          {
           if(OrderStopLoss() < Bid-PP_RSI_PS_stop*MyPoint)
            {
             bool modifying;
             modifying = OrderModify(OrderTicket(),OrderOpenPrice(),NormalizeDouble((Bid-PP_RSI_PS_stop*MyPoint),Digits),OrderTakeProfit(),0,Green);   
            }
          }
           }
         else
          ...
          if(iRSI(NULL,PP_RSI_PS_Time,14,PRICE_CLOSE,2) < PP_RSI_PS_low && iRSI(NULL,PP_RSI_PS_Time,14,PRICE_CLOSE,1) > PP_RSI_PS_low)
            {
               ...
               {
                bool modifying;
                modifying = OrderModify(OrderTicket(),OrderOpenPrice(),NormalizeDouble((Ask+PP_RSI_PS_stop*MyPoint),Digits),OrderTakeProfit(),0,Red);
                }
             }
          }
        }  
         ...
         if(iSAR(NULL,PP_PSAR_PS_Time,PP_PSAR_PS_step,PP_PSAR_PS_maxstep,2)<Ask && iSAR(NULL,PP_PSAR_PS_Time,PP_PSAR_PS_step,PP_PSAR_PS_maxstep,1)>Ask) 
          {
           if(OrderStopLoss() < Bid-PP_PSAR_PS_stop*MyPoint)
            {
             bool modifying;
             modifying = OrderModify(OrderTicket(),OrderOpenPrice(),NormalizeDouble((Bid-PP_PSAR_PS_stop*MyPoint),Digits),OrderTakeProfit(),0,Green);   
            }
          }
           }
         else
         ...
         if(iSAR(NULL,PP_PSAR_PS_Time,PP_PSAR_PS_step,PP_PSAR_PS_maxstep,2)>Ask && iSAR(NULL,PP_PSAR_PS_Time,PP_PSAR_PS_step,PP_PSAR_PS_maxstep,1)<Ask)
            {
               ...
               {
                bool modifying;
                modifying = OrderModify(OrderTicket(),OrderOpenPrice(),NormalizeDouble((Ask+PP_PSAR_PS_stop*MyPoint),Digits),OrderTakeProfit(),0,Red);
                }
             }
          }
        }  
           ...
           if(iADX(NULL,PP_ADX_PS_Time,14,PRICE_CLOSE,MODE_MAIN,2) > PP_ADX_PS_level && iADX(NULL,PP_ADX_PS_Time,14,PRICE_CLOSE,MODE_MAIN,1) < PP_ADX_PS_level)
             {
               ...
               {
                bool modifying;
                modifying =  OrderModify(OrderTicket(),OrderOpenPrice(),NormalizeDouble((Bid-PP_ADX_stop*MyPoint),Digits),OrderTakeProfit(),0,Green);  
                }
             }
           }
         else
         ...
         if(iADX(NULL,PP_ADX_PS_Time,14,PRICE_CLOSE,MODE_MAIN,2) > PP_ADX_PS_level && iADX(NULL,PP_ADX_PS_Time,14,PRICE_CLOSE,MODE_MAIN,1) < PP_ADX_PS_level)
            {
               ...
               {
                bool modifying;
                modifying = OrderModify(OrderTicket(),OrderOpenPrice(),NormalizeDouble((Ask+PP_ADX_stop*MyPoint),Digits),OrderTakeProfit(),0,Red);
                }
             }
          }
        }
           ...
           if(MACDmainPrevious > MACDsignalPrevious && MACDmainCurrent < MACDsignalCurrent) //sell signal
             {
               ...
               {
                bool modifying;
                modifying = OrderModify(OrderTicket(),OrderOpenPrice(),NormalizeDouble((Bid-PP_MACD_PS_stop*MyPoint),Digits),OrderTakeProfit(),0,Green);  
                }
             }
           }
         else
         ...
         if(MACDmainPrevious < MACDsignalPrevious && MACDmainCurrent > MACDsignalCurrent) //buy signal
            {
               ...
               {
                bool modifying;
                modifying = OrderModify(OrderTicket(),OrderOpenPrice(),NormalizeDouble((Ask+PP_MACD_PS_stop*MyPoint),Digits),OrderTakeProfit(),0,Red);
                }
             }
          }
        }
           ...
           if (Bid-OrderOpenPrice() > PP_Breakeven_Point*MyPoint)
             {
               if(OrderStopLoss() < PP_Breakeven_LockIn*MyPoint+OrderOpenPrice()) 
                 { 
                  bool modifying;
                  modifying = OrderModify(OrderTicket(),OrderOpenPrice(),NormalizeDouble((OrderOpenPrice()+PP_Breakeven_LockIn*MyPoint),Digits),OrderTakeProfit(),0,Green);   
                  }
              }   
              }   
        else
         ...
         if (OrderOpenPrice()-Ask > PP_Breakeven_Point*MyPoint)
            {
             if(OrderStopLoss() > (OrderOpenPrice()-PP_Breakeven_LockIn*MyPoint)) 
               {
                  bool modifying;
                  modifying =  OrderModify(OrderTicket(),OrderOpenPrice(),NormalizeDouble((OrderOpenPrice()-PP_Breakeven_LockIn*MyPoint),Digits),OrderTakeProfit(),0,Red);
               }
